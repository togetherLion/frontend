...
//         axios.post("http://172.30.1.81:8080/user/userProfile", { userId: userId }),
//         axios.get("http://172.30.1.81:8080/posts/my"),
//         axios.get("http://172.30.1.81:8080/goods/liked"),
//       ]);
...
const MyPage = ({ navigation, route }) => {
  ...
  const [nickname, setNickname] = useState('');
  const [townName, setTownName] = useState('');
  const [profilePicture, setProfilePicture] = useState('https://via.placeholder.com/70x70.png?text=User');
  const [myPost, setMyPost] = useState([]);
  const [likedPost, setLikedPost] = useState([]);
  const [loading, setLoading] = useState(true);

  // 아이템 컴포넌트 (가로 썸네일 리스트에 사용) - memo로 불필요 렌더 최소화
  ...
  const renderMyPost = useCallback(
    ({ item, index }) => (
      <ItemCard
        item={item}
        onPress={() => navigation.navigate('PostDetail', { postId: item.postId, userId })}
      />
    ),
    [navigation, userId]
  );

  const renderLikedPost = useCallback(
    ({ item, index }) => (
      <ItemCard
        item={item}
        onPress={() => navigation.navigate('PostDetail', { postId: item.postId, userId })}
  ...
  // 가로 카드의 고정 크기를 FlatList에 알려주면 스크롤/측정 비용 ↓
  const getItemLayoutHorizontal = useCallback((data, index) => {
    const ITEM_WIDTH = 108; // 카드 100 + marginRight 8
    return { length: ITEM_WIDTH, offset: ITEM_WIDTH * index, index };
  }, []);

  // 이미지 미리 불러오기(선택) - 대량이면 생략 가능
  const prefetchImages = useCallback(async (list) => {
    if (!list || list.length === 0) return;
    const tasks = list
      .filter(p => !!p.postPicture)
      .map(p => Image.prefetch(p.postPicture).catch(() => {}));
  ...
  async function getMyPage() {
    ...
    try {
      const requests = [
        axios.post('http://172.30.1.81:8080/user/userProfile', { userId }, { timeout: 8000 }),
        axios.get('http://172.30.1.81:8080/posts/my', { timeout: 8000 }),
        axios.get('http://172.30.1.81:8080/goods/liked', { timeout: 8000 }),
      ];
      ...
      if (resp1?.data) {
        setNickname(resp1.data.nickname ?? '');
        setTownName(resp1.data.townName ?? '');
        setProfilePicture(resp1.data.profilePicture || 'https://via.placeholder.com/70x70.png?text=User');
      }

      const my = Array.isArray(resp2?.data) ? resp2.data : [];
      const liked = Array.isArray(resp3?.data) ? resp3.data : [];

      setMyPost(my);
      setLikedPost(liked);

      // (선택) 이미지 미리 불러오기
      prefetchImages([...my, ...liked]);
    } catch (err) {
      console.log('마이페이지 API 에러:', err?.message || err);
    } finally {
      setLoading(false);
      console.log('마이페이지 전체 소요(ms):', Date.now() - startAll);
    }
  }
  ...
  const confirmLogout = () => {
    axios.post('http://165.229.169.121:8080/user/logout', {}, { timeout: 8000 })
      .then((resp) => {
        if (resp?.data) {
          setModalVisible(false);
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.scrollViewContentContainer}>
        ...
        <View style={styles.profileContainer}>
          ...
          <View style={styles.profileTextContainer}>
            <Text style={styles.profileName}> {nickname} ({townName})</Text>
            <View style={styles.buttonContainer}>
              <TouchableOpacity style={styles.editButton} onPress={() => navigation.navigate('Profile', { userId })}>
                <Text style={styles.editButtonText}>내 프로필 보기</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.editButton} onPress={() => navigation.navigate('UserInform')}>
                <Text style={styles.editButtonText}>회원 정보 보기</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          {myPost && myPost.length > 0 ? (
            <FlatList
              ...
              keyExtractor={(item, index) =>
                (item?.postId && String(item.postId)) || (item?.id && String(item.id)) || String(index)
              }
              initialNumToRender={6}
              maxToRenderPerBatch={6}
              windowSize={7}
              removeClippedSubviews
              getItemLayout={getItemLayoutHorizontal}
              showsHorizontalScrollIndicator={false}
            />
          ) : (
            <Text style={styles.noContentMessage}>작성한 공구글이 없습니다</Text>
...
const styles = StyleSheet.create({
  bigcontainer: { flex: 1, backgroundColor: '#fff' },
  ...
  header: { width: '100%', alignItems: 'flex-start', marginBottom: 50, marginTop: 15 },
  profileContainer: { flexDirection: 'row', alignItems: 'center', padding: 16 },
  profileImage: { width: 70, height: 70, borderRadius: 30 },
  profileTextContainer: { marginLeft: 16 },
  profileName: { fontSize: 18, fontWeight: 'bold' },
  buttonContainer: { flexDirection: 'row' },
  editButton: {
    marginTop: 10, marginRight: 20, padding: 8, borderRadius: 7, borderColor: 'black',
  },
  editButtonText: { color: 'black', textAlign: 'center' },
  section: { padding: 16, borderBottomWidth: 1, borderBottomColor: '#ccc' },
  sectionTitle: { fontSize: 16, fontWeight: 'bold', marginBottom: 15 },
  subTitle: { marginTop: 4, color: '#888' },
  item: { marginRight: 8, width: 100 }, // getItemLayout 가정치에 맞춰 폭 고정
  itemImage: { width: 100, height: 100, borderRadius: 8 },
  itemTitle: { marginTop: 8, fontSize: 14, fontWeight: 'bold' },
  itemPrice: { marginTop: 4, color: '#888' },
  footer: { flexDirection: 'row', borderTopWidth: 1, borderTopColor: '#ccc', padding: 16 },
  footerButton: { flex: 1 },
