...
const PostDetail = ({ route }) => { // navigation 제거
    ...
    const handleHeartPress = () => {
        if (!isgood) {
            axios.post("http://172.30.1.81:8080/goods/postlike", { postId: postId });
        }
        else {
            axios.delete("http://172.30.1.81:8080/goods", {
                params: {
                    postId: postId
                }
            });
    ...
    async function getPostDetail() {
        try {
            const responses = await Promise.all([
                axios.get(`http://172.30.1.81:8080/posts/${postId}`),
                axios.get(`http://172.30.1.81:8080/posts/user-posts/${postId}`),
            ]);
    ...
    const confirmParticipate = () => {
        axios.post("http://172.30.1.81:8080/waitingdeal", { postId: postId })
            .then((resp) => {
    ...
    const confirmDelete = () => {
        axios.delete(`http://172.30.1.81:8080/posts/${postId}`)
            .then((resp) => {
