...
const ListSearchScreen = ({ navigation, route }) => {
  ...
  // 날짜 포맷: 2025-11-16T10:23:00 -> 2025.11.16
  const formatDate = (dateString) => {
    if (!dateString) return '';
    const d = new Date(dateString);
    if (isNaN(d.getTime())) return ''; // 파싱 실패 시 빈 문자열
      ...
      <View style={styles.postItem}>
        ...
        <View style={styles.postContent}>
          ...
          <View style={styles.postInfo}>
            <Text style={styles.postAuthor}>{post.author}</Text>
            <Text style={styles.postLocation}>{post.location}</Text>
            <Text style={styles.postDeadline}>{formatDate(post.deadlineDate)}</Text>
          </View>
          {post.additionalInfo && <Text style={styles.additionalInfo}>{post.additionalInfo}</Text>}
        </View>
      </View>
    ...
    <View style={styles.container}>
      ...
      <FlatList
        ...
        keyExtractor={(item) =>
          ...
          (item.postId && item.postId.toString()) ||
