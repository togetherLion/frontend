...
const Profile = ({ navigation, route }) => {
  ...
  async function getProfile() {
    try {
      ...
      const responses = await Promise.all([
        axios.post("http://192.168.219.45:8080/user/userProfile", {
          userId: userId,
        }),
        axios.get(`http://192.168.219.45:8080/review/user/${userId}`),
      ]);
  ...
  const confirmFollow = () => {
    axios.post("http://192.168.219.45:8080/follow", { userId: userId })
      .then((resp) => {
  ...
  const confirmUnfollow = () => {
    axios.post("http://192.168.219.45:8080/follow/unfollow", { userId: userId })
      .then((resp) => {
        console.log(resp.data);
        if (resp.data !== null && resp.data !== "") {
          setModalVisible(false);
