...
const ChatButtonScreen = ({ navigation, route }) => {
  ...
  const checkCreateChat = async () => {
    try {
      const response = await axios.get(
        'http://165.229.169.110:8080/waitingdeal/accepted-users?postId=' + postId/*+ 현재 postId로 바꾸기*/
      )
  ...
  const createChat = async (postId, roomName) => {
    try {
      const response = await axios.post(
        'http://165.229.169.110:8080/chat/check-and-create-room?' +
          'postId=' +
          postId +
          '&roomName=' +
          roomName
  ...
  const sendWaiting = async () => {
    try {
      await axios.post('http://165.229.169.110:8080/waitingdeal', {
