...
const SignupScreen = ({ navigation, route }) => {
  ...
  const checkId = () => {
    if (loginId.trim() === "") {
      ...
      axios.post("http://165.229.169.110:8080/user/idCheck", {
        loginId: loginId
  ...
  const checkNickname = () => {
    if (nickName.trim() === "") {
      ...
      axios.post("http://165.229.169.110:8080/user/nicknameCheck", {
        nickname: nickName,
      }).then(function (response) {
        console.log(response.data);  // 서버에서 받은 응답을 콘솔에 출력합니다.
  ...
  const handleSubmit = () => { // 회원가입할때
    if (!isIdAvailable) {
      ...
      axios.post("http://165.229.169.110:8080/user/signup", {
        loginId: loginId,
        password: password,
        name: name,
        nickname: nickName,
